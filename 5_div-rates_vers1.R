...
##############################################################################################################
################################################################################
# Div by time of the growth rates
...
#data retrieval
for(k in ( 1 : Nbrow)) {
  dummylength <- ncol(rawdata2[k, 751 : 1500])
  temp <- as.data.frame((matrix(nrow = dummylength, ncol = 1)))
  temp$Div <-  t(rawdata2[k, 751 : 1500])
...
#data retrieval
for(k in ( 1 : Nbrow)) {
  dummylength <- ncol(rawdata3[k, 751 : 1500])
  temp <- as.data.frame((matrix(nrow = dummylength, ncol = 1)))
  temp$Div <-  t(rawdata3[k, 751 : 1500])
...
#data retrieval
for(k in ( 1 : Nbrow)) {
  dummylength <- ncol(rawdata4[k, 751 : 1500])
  temp <- as.data.frame((matrix(nrow = dummylength, ncol = 1)))
  temp$Div <-  t(rawdata4[k, 751 : 1500])
...
#data retrieval
for(k in ( 1 : Nbrow)) {
  dummylength <- ncol(rawdata5[k, 751 : 1500])
  temp <- as.data.frame((matrix(nrow = dummylength, ncol = 1)))
  temp$Div <-  t(rawdata5[k, 751 : 1500])
...
#data retrieval
for(k in ( 1 : Nbrow)) {
  dummylength <- ncol(rawdata6[k, 751 : 1500])
  temp <- as.data.frame((matrix(nrow = dummylength, ncol = 1)))
  temp$Div <-  t(rawdata6[k, 751 : 1500])
...
#data retrieval
for(k in ( 1 : Nbrow)) {
  dummylength <- ncol(rawdata7[k, 751 : 1500])
  temp <- as.data.frame((matrix(nrow = dummylength, ncol = 1)))
  temp$Div <-  t(rawdata7[k, 751 : 1500])
...
#data retrieval
for(k in ( 1 : Nbrow)) {
  dummylength <- ncol(rawdata8[k, 751 : 1500])
  temp <- as.data.frame((matrix(nrow = dummylength, ncol = 1)))
  temp$Div <-  t(rawdata8[k, 751 : 1500])
...
#data retrieval
for(k in ( 1 : Nbrow)) {
  dummylength <- ncol(rawdata9[k, 751 : 1500])
  temp <- as.data.frame((matrix(nrow = dummylength, ncol = 1)))
  temp$Div <-  t(rawdata9[k, 751 : 1500])
...
write.csv(longdata, file = ".\\longdata\\longdata-div.csv")
longdata <- read.csv(filenames[2])

dataDivc <- c()
dataDivc <- summarySE(longdata, 
                       measurevar="Div", 
                       groupvars= c("V1", "Treat"), 
                       na.rm = TRUE)

dataDivc <- subset(dataDivc, !is.na(dataDivc$sd))
dataDivc$Treat <- factor(dataDivc$Treat, 
...
dataDivc$V1 <- dataDivc$V1 * 0.2


###########################################################################
###########################################################################
limits <- aes(ymax = Div + ci, ymin = Div - ci)
ggplot(dataDivc[10 : 3706,], aes(V1, Div)) + geom_point(color = "chartreuse3")+
  facet_wrap(~Treat, ncol = 3, scales = "fixed")+
  geom_errorbar(limits, width = 0.2) +
  ylim(0.1, 0.6) + 
  xlim(0, 200) +
  labs(x="Time (hours)", y = "Division rate") + 
  ggtitle(expression(atop("Division rate", atop(italic("N = 10420 Cells"), "")))) +
  
  geom_ribbon(aes(ymin=Div-ci, ymax=Div+ci), alpha=0.1, fill="chartreuse3") +
